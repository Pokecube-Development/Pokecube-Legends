...
{	
	...
	public UltraSpacePortal(String name, Material material, double posX, double posY, double posZ, double sizeX, double sizeY, double sizeZ) 
	{
		super(name, material);
        ...
        if (!playerIn.isRiding() && !playerIn.isBeingRidden() && playerIn.isNonBoss()) {
            if (!playerIn.world.isRemote && playerIn instanceof EntityPlayerMP) {
                ...
                if (player.timeUntilPortal > 0) {
                    ...
                    if (dimensionId == DimensionInit.ULTRASPACE.getId()) {
                        dimensionIn = 0;
                    } else {
                        dimensionIn = DimensionInit.ULTRASPACE.getId();
                    }

                    player.timeUntilPortal = 5;
                    minecraftServer.getPlayerList().transferPlayerToDimension(player, dimensionIn, new TeleportUltraSpace(minecraftServer.getWorld(dimensionIn)));
                }
            }
        }
        return false;
