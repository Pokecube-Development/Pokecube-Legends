...
public class HeatranSpawn
 {
    ...
    public void interactRightClickBlock(PlayerInteractEvent.RightClickBlock evt)
    {
        boolean invalid = !evt.getEntityPlayer().isSneaking() || !CompatWrapper.isValid(evt.getItemStack())
                || !(evt.getItemStack().getItem() instanceof MagmaCore /*ItemPokedex*/) || evt.getWorld().isRemote;
        if (invalid) return;
        Block block = null;
        ...
        PokedexEntry entry = Database.getEntry("heatran");
        if (block == Blocks.MAGMA && entry != null)
        {
