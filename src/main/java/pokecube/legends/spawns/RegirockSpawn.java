...
public class RegirockSpawn
 {
    ...
    public void interactRightClickBlock(PlayerInteractEvent.RightClickBlock evt)
    {
        boolean invalid = !evt.getEntityPlayer().isSneaking() || !CompatWrapper.isValid(evt.getItemStack())
                ||!(evt.getItemStack().getItem() instanceof RockCore /*ItemPokedex*/) || evt.getWorld().isRemote;
        if (invalid) return;
        Block block = null;
        ...
        PokedexEntry entry = Database.getEntry("regirock");
        if (block == BlockInit.REGIROCK_CORE && entry != null)
        {
