...
public class XerneasSpawn
 {
    ...
    public void interactRightClickBlock(PlayerInteractEvent.RightClickBlock evt)
    {
        boolean invalid = !evt.getEntityPlayer().isSneaking() || !CompatWrapper.isValid(evt.getItemStack())
        		|| !(evt.getItemStack().getItem() instanceof LifeOrb /*ItemPokedex*/) || evt.getWorld().isRemote;
        if (invalid) return;
        Block block = null;
        ...
        PokedexEntry entry = Database.getEntry("xerneas");
        if (block == BlockInit.XERNEAS_CORE && entry != null)
        {
